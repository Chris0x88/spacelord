...
class PacmanChat:
    ...
    def handle_swap(self, intent: Dict) -> str:
        ...
        if self.config.simulate_mode:
            lines.append("   (No real transaction - add private key to .env for live trading)")
...
💡 To enable live trading:
   ...
   3. Set PACMAN_SIMULATE=false
"""
            else:
                # Would execute live transaction here
                return "🚀 Live execution would happen here (implementation pending)"
...
Setup:
  1. Copy .env.template to .env
  2. Add PACMAN_PRIVATE_KEY=your_key
  3. Set HEDERA_ACCOUNT_ID=0.0.xxx
  4. Set PACMAN_SIMULATE=false for live trading

Current Mode: {mode}
Max Amount: $1.00 per swap
""".format(mode="SIMULATION" if self.config.simulate_mode else "LIVE")
...
def run_interactive():
    ...
    # Demo
    print("🎮 DEMO - Testing: 'swap 1 usdc for wbtc'")
    print(chat.chat("swap 1 usdc for wbtc"))
    print("\n" + "-"*60)
