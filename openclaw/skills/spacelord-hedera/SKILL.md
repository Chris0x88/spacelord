...
metadata:
  openclaw:
    ...
    requires:
      ...
      env: [PRIVATE_KEY, HEDERA_ACCOUNT_ID, SPACELORD_NETWORK]
    configPaths: ["./data/", "./.env"]
    os: [darwin, linux]
---

> **Runtime:** This skill drives `./launch.sh`, a zero-dependency bash launcher included in the spacelord-hedera repository. It installs `uv` on first run, enforces `.env` existence, and dispatches to `uv run python -m cli.main`. All credentials stay local in `.env` — nothing is transmitted to external endpoints except Hedera mainnet RPC and SaucerSwap API.
...
| "when does floor hit X?" / "forecast [date]" / "break-even on power law" | `forecast` command — date or price mode |
...
Any error
  ...
  +- "command not found: spacelord" -> Use `./launch.sh`, not `./spacelord`
...
- Live liquidity from `data/spacelord_data_raw.json` enhances depth information
...
| Command | Syntax | Purpose | Flags |
|---------|--------|---------|-------|
| `bridge` | `bridge <amt> USDT0 <chain> --to <addr>` | Bridge USDT0 via LayerZero | `--json` |
| `bridge status` | `bridge status <tx_hash>` | Check bridge delivery status | `--json` |
| `bridge whitelist list` | `bridge whitelist list` | Show whitelisted bridge destinations | — |
| `bridge whitelist add` | `bridge whitelist add <chain> <addr> <label>` | Add bridge destination | — |

**Supported chains:** arbitrum

**Examples:**
- `bridge whitelist add arbitrum 0xYourAddr My Arb Wallet` — Whitelist a destination first
- `bridge 50 USDT0 arbitrum --to 0xYourAddr` — Bridge 50 USDT0 to Arbitrum
- `bridge status 0xabc123...` — Check LayerZero delivery status

**Notes:** Destination must be whitelisted. Counts toward daily transaction limit. Estimated arrival: 30 seconds to 3 minutes.
...
| `robot signal` | `robot signal` | BTC Power Law model snapshot for TODAY | `--json` |
| `robot start` | `robot start` | Start rebalancer (needs >= $5) | — |
| `robot stop` | `robot stop` | Stop rebalancer | — |

### FORECAST (Power Law Strategic Projections)

**Two modes — use `forecast` when the user asks about future dates or price targets:**

| Command | Syntax | Purpose | Flags |
|---------|--------|---------|-------|
| `forecast <date>` | `forecast 01/07/2026` | Floor/model/ceiling at a future date vs live BTC | `--json` |
| `forecast <price>` | `forecast $66000` | When does each model line reach this price? | `--json` |

**Mode 1 — Date forward** (`forecast 01/07/2026`):
- Accepted formats: `DD/MM/YYYY`, `YYYY-MM-DD`, `DD/MM/YY`
- Shows floor, model price, and ceiling at that date
- Shows % change from today's live BTC price to each level
- Shows cycle phase, valuation regime, recommended allocation at that date
- Use when user asks: "what does the model say in July?", "what's the ceiling next year?"

**Mode 2 — Price reverse** (`forecast $66000`):
- Strip `$` and commas; bare number also works (`forecast 66000`)
- Shows when floor, model price, and ceiling each first reach the target
- Floor uses binary search (monotonic). Model/ceiling use day-by-day (non-monotonic due to cycle)
- Use when user asks: "when does the floor hit $X?", "if I buy at $66K when does the model catch up?", "break-even on the power law floor"
- **Key use case**: User buys BTC at a given price and wants to know when the lower bound catches up = model break-even date

**Decision rule — `robot signal` vs `forecast`:**
- `robot signal` → user wants today's snapshot (allocation, valuation, current signal)
- `forecast` → user asks about a specific future date OR a specific price level
...
```json
{
  "accounts": [
    {
      "account": "<HEDERA_ACCOUNT_ID>", "role": "main", "nickname": "Main Transaction Account",
      "hbar": {"balance": 55.21, "price_usd": 0.093, "value_usd": 5.12},
      "tokens": {"USDC": {"balance": 6.58, "price_usd": 1.0, "value_usd": 6.58}},
      "total_usd": 12.22
    },
    {
      "account": "<ROBOT_ACCOUNT_ID>", "role": "robot", "nickname": "Bitcoin Rebalancer Daemon",
...
👤 **Main** — `<HEDERA_ACCOUNT_ID>`
  ⟐ HBAR   `57.84`  ~ $10.99
  💵 USDC   `6.58`   ~ $6.58
─ ─ ─  💼 Subtotal **$17.57**

🤖 **Robot** — `<ROBOT_ACCOUNT_ID>`
  ⟐ HBAR   `39.21`  ~ $3.64
  💵 USDC   `11.50`  ~ $11.50
  ₿ WBTC   `0.00024` ~ $16.71
─ ─ ─  💼 Subtotal **$31.85**
...
# SECTION 18: WHAT MAKES SPACE LORD SPECIAL
